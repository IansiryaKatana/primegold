    ...
    <>
      ...
      >
        <Button variant="gold" asChild>
          <AppLink href={links.estimate}>{sellCopy.estimateCta}</AppLink>
        </Button>
        <Button variant="outlineGold" asChild>
          <AppLink href={links.locations}>{sellCopy.appointmentCta}</AppLink>
        </Button>
      </PageHero>
      ...
      <MotionSection tier="b" className="bg-white py-16 md:py-20">
        <Container>
          <RevealBlock>
            <h2 className="text-subheading text-center text-primary-text">{sellCopy.process.title}</h2>
          </RevealBlock>
            ...
            <TabsContent value="accepted">
              <ul className="mt-8 grid gap-3 sm:grid-cols-2">
                {sellCopy.accepted.map((item) => (
                  <li
                    key={item}
                    className="surface-feature flex items-center gap-3 px-4 py-3 text-sm text-primary-text md:text-base"
                  >
            ...
            <TabsContent value="rejected">
              <ul className="mt-8 grid gap-3 sm:grid-cols-2">
                {sellCopy.rejected.map((item) => (
                  <li
                    key={item}
                    className="surface-inset flex items-center gap-3 px-4 py-3 text-sm text-muted-text md:text-base"
                  >
